...
async function main() {
     ...
     */
    const uri = "mongodb+srv://trivedi007:admin@cluster0.cijqtcr.mongodb.net/?retryWrites=true&w=majority";
    ...
    try {
        ...
        await findByName(client, "Jane");
        // Update the Infinite Views listing to have 6 bedrooms and 8 beds 
        await updateByName(client, "https://aadhar/minor/adress/MH?redirectfrom=directory", "Jane");
 ...
 */
async function updateByName(client,value,coditionValue) {
    // See https://mongodb.github.io/node-mongodb-native/3.6/api/Collection.html#updateOne for the updateOne() docs
    const result = await client.db("sample_airbnb").collection("listingsAndReviews").updateOne({ "name.given": coditionValue }, { $set: {"document.aadhar.uploadURL":value} });
 ...
 * If a listing with the given name git pushexists, it will be updated.
...
async function findByName(client, value){
    const result = await client.db("sample_airbnb").collection("listingsAndReviews").findOne({ "name.given": value });

    if (result) {
        console.log(`Found a aadhar url '${result.document.aadhar.uploadURL}':`);
        // console.log(result);
    } else {
